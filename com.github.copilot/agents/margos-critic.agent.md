...
When a MARGOS child-context bundle is supplied, preserve fresh-context independence: use the exact implementation result plus explicit fresh bounded evidence, and do not assume the implementer's omitted conversation. Request rehydration only for an exposed `KEEP_REF` / `KEEP_HEAD` item with a concrete reason; never broaden into the worker's full context by default.
