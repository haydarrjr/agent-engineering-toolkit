...
The live command requires at least 300 routing and 300 context cases, resolves `/v1/models`, prefers pinned `jev-1.13.0`, and returns exactly one promotion status. Without `TYPESAFE_API_KEY`, it returns `live_status=NOT_RUN` and `JEV_NOT_PROMOTED` and must not be merged. Smaller fixture invocations are local smoke tests only. `JEV_PROMOTED_FOR_FROZEN_SUITE` means the authorized run passed the frozen protocol's safety, non-inferiority, zero-provider-error, pinned-response-model, partition, and runtime-fingerprint gates; it does not freeze the alias forever or remove Policy veto. A model, question, threshold, projection, extractor, corpus, or benchmark-protocol change requires a new calibration/promotion run.

The Issue #18 authorized run on source `b78c26a` requested `jev-latest` and observed response model `jev-1.13.0`. It evaluated 300 routing and 300 context cases per forced arm, produced zero provider errors after bounded adapter retries, and returned `JEV_PROMOTED_FOR_FROZEN_SUITE`. JEV is therefore active as an optional typed Reflex in MARGOS; Policy-only remains the deterministic fallback whenever admission skips, the host cannot exploit the result, the provider is disabled/unavailable, a budget applies, or final Policy veto rejects the proposal.
