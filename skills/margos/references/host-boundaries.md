...
For a requested profile/configuration change, inspect owned state, plan the smallest create/replace/no-op/conflict result, obtain any confirmation required for that mutation, apply only the requested scope, then verify readback. Never infer capability from a profile, manifest, model label, or previous success alone.
