...
Route by required capability and evidence, not prompt length, file count, model brand, or keywords.
...
- **FRONTIER_REASONING** — unresolved contradictions, broad architectural/cross-system consequences, high-impact correctness/security obligations, difficult synthesis that cannot be decomposed safely, or a lower-tier result that failed relevant verification.

`INDEPENDENT_CRITIC` is a role, not a compute tier. Use the smallest compute tier that can credibly perform fresh-context falsification.
...
Do not escalate merely because a task says "research", "review", contains many files, has a long prompt, or mentions a model name. Do not keep a frontier child alive after its hard question is closed. Re-evaluate the next independent subtask from the lowest sufficient class.
