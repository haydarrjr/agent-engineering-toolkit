...
The normal routing CLI can explicitly select the adapter with `--reflex-provider jev`; setting `TYPESAFE_API_KEY` alone still does nothing. Routing and Context Reflex questions point to paths relative to the request's top-level `state` object (`task...`, `admissible...`, `candidates[0]...`). The adapter must not emit a redundant `state.` prefix: TypeSafe resolves paths inside `state`, and the extra prefix makes the question indirect or ungrounded.

`jev-latest` remains the research default and is resolved at benchmark time; the response fingerprint must record the concrete model. The current TypeSafe lifecycle documents the alias as pointing to `jev-1.13.0`, but an alias is not itself a calibration binding. A calibrated run must supply a `margos-jev-calibration/v2` binding that matches the observed model, provider, question-set hash, threshold-policy hash, projection version, frozen-corpus hash, and a recorded `PASSED` evaluation. Alias/model/contract drift yields `STALE`, not a calibration claim.

Admission is outcome-aware. If deterministic Policy fixes a compute floor from hard verification evidence, or the host cannot exploit a different route, the adapter is not called. The benchmark must measure downstream verification and end-to-end latency/cost; route-label agreement alone is not verified success and cannot promote JEV.
