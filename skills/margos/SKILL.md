...
The default contract helper is [margos_decide.py](scripts/margos_decide.py). Its deterministic/provider-neutral path is offline by default; an external Reflex adapter is used only when the caller explicitly selects one. The offline context helper is [margos_context.py](scripts/margos_context.py); it materializes derived Context Views without mutating canonical evidence. External providers are optional, fail closed, and are never required for ordinary MARGOS use. Finish with the requested result, host evidence for delegated work, and unresolved capability or reconciliation state. MARGOS remains explicit-only for ordinary engineering work.
